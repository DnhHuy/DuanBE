...
// Nếu không tìm thấy user, hãy gọi hàm callback với tham số: callback("Không tìm th́ây user!", null) .
...
// Yêu cầu:
// 1. Viết một hàm mới tên là getUserByIdPromise(id) . Hàm này không nhận vào tham số callback nữa, mà sẽ trả về một new
...
// Nếu không tìm thấy, hãy kích hoạt hành động thất bại bằng cách gọi: reject("Không tìm th́ây user!") .
...
getUserByIdPromise(2)
  .then((user) => console.log("Bài 2 - Tìm th́ây user:", user))
  .catch((error) => console.error("Lô ̃i:", error));
...
// Mục tiêu: Sử dụng cú pháp async/await kết hợp try...catch để viết code bất đồng bộ trông giống như code đồng bộ.
// Ðây là cách viết phổ biến nhất trong Node.js thực tế.
// Yêu cầu:
// 1. Viết một hàm tên là runApp(userId) . Hãy đánh dấu hàm này là một hàm bất đồng bộ bằng từ khóa async .
// 2. Bên trong hàm, sử dụng từ khóa await để gọi và lấy trực tiếp kết quả từ hàm getUserByIdPromise(userId) đã viết ở Bài
// 2.
// 3. Bọc toàn bộ quá trình thực thi trong khối lệnh try...catch để xử lý lỗi một cách an toàn nếu Promise bị reject

function runApp(userId) {
  return new Promise(async (resolve, reject) => {
    try {
      const user = await getUserByIdPromise(userId);
      console.log("Bài 3 - Tìm thấy user:", user);
      resolve(user);
    } catch (error) {
      console.error("Bài 3 - Kết quả mong muốn (Báo lỗi):", error);
      reject(error);
    }
  });
}
...
async function runApp(userId) {
  try {
    console.log(`--- Đang tìm kíêm user có ID: ${userId} ---`);
    const user = await getUserByIdPromise(userId);
    console.log("Bài 3 - Kết quả:", user);
  } catch (error) {
    console.error("Bài 3 - Bắt được lỗi hệ th́ông:", error);
  }
}
// Gọi hàm chạy thư ̉
runApp(1); // Trường hợp thành công
runApp(55); // Trường hợp th́ât bại (Sẽ chạy vào kh́ôi catch)
